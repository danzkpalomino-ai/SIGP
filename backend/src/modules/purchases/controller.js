...
export async function createPurchase(req, res) {
  try {
    ...
    const purchase = await Purchase.create({
      correlativo, tipo_documento: '01', serie: 'C001', numero: String(correlativo).padStart(6, '0'),
      proveedor_nombre, proveedor_ruc, fecha_emision: new Date().toISOString().split('T')[0], subtotal, igv, total: totalFinal,
      items: items.map(it => ({ ...it, descripcion: it.descripcion?.toUpperCase(), total_item: it.total_item || it.cantidad * it.precio_unitario })),
      company_id: req.company_id, registrado_por: req.user?._id, origen: 'sigp', tipo_compra: tipo_compra || 'mercadera'
    });
...
export async function updatePurchase(req, res) {
  try {
    const db = getTenantDb(req.company_id);
    const Purchase = db.model('Purchase');
    const purchase = await Purchase.findOneAndUpdate({ _id: req.params.id, company_id: req.company_id }, { $set: req.body }, { new: true });
