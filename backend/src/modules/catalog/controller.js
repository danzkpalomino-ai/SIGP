...
export async function createProduct(req, res) {
  try {
    ...
    const product = await Product.create({
      descripcion: descripcion.toUpperCase(), marca: marca?.toUpperCase(), categoria: categoria?.toUpperCase(),
      modulo_pos: modulo_pos?.toUpperCase(), precio_unitario: Number(precio_unitario) || 0,
      stock_actual: Number(stock_actual) || 0, item_type: item_type || 'PRODUCTO',
      codigo_pos, codigo_barra, imagen: imagen || undefined,
...
export async function getProductByCode(req, res) {
  try {
    const db = getTenantDb(req.company_id);
    const Product = db.model('Product');
    const product = await Product.findOne({ company_id: req.company_id, $or: [{ codigo_pos: req.params.code }, { codigo_barra: req.params.code }] });
...
export async function updateProduct(req, res) {
  try {
    const db = getTenantDb(req.company_id);
    const Product = db.model('Product');
    const product = await Product.findOneAndUpdate({ _id: req.params.id, company_id: req.company_id }, { $set: req.body }, { new: true });
    if (!product) return res.status(404).json({ message: 'Producto no encontrado' });
    res.json(product);
  } catch (err) { res.status(500).json({ message: err.message }); }
}
