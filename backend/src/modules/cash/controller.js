...
export async function openShift(req, res) {
  try {
    ...
    const todaySales = await Sale.find({ company_id: req.company_id, origen: 'sigp', fecha_emision: today, estado: 'COMPLETADO' });
    const ventas_total = todaySales.reduce((acc, s) => acc + (s.total || 0), 0);
    const ventas_count = todaySales.length;
    const shift = await Shift.create({ cajero_id: req.user._id, cajero_nombre: req.user.username, monto_inicial, notas, ventas_total, ventas_count, company_id: req.company_id });
