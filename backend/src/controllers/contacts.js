...
export async function createContact(req, res) {
  try {
    ...
    if (!razon_social || !ruc_dni || !type) {
      return res.status(400).json({ message: 'razon_social, ruc_dni y type son requeridos' });
    }
    ...
    if (err.code === 11000) return res.status(409).json({ message: 'El contacto ya existe' });
    res.status(500).json({ message: err.message });
  }
}
