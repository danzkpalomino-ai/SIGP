...
export default function SuppliersView() {
  ...
  const loadSuppliers = async () => {
    ...
    try {
      ...
      setSuppliers(res.data.contacts || [])
  ...
  const handleCreate = () => {
    setForm({ razon_social: '', ruc_dni: '', telefono: '', email: '', direccion: '', type: 'PROVEEDOR' })
    setSelected(null)
    setShowForm(true)
    setError('')
  }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="glass px-6 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="relative max-w-md">
            ...
            <input className="glass-input w-full pl-9 pr-3 py-2 rounded-lg text-[13px]" placeholder="BUSCAR POR NOMBRE O RUC..." value={search} onChange={e => setSearch(e.target.value.toUpperCase())} />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto p-6" style={{ background: 'rgba(10,10,11,0.6)' }}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              <Truck size={48} style={{ color: '#2A2A2E' }} className="mb-4" />
              <p className="text-sm font-bold uppercase tracking-widest" style={{ color: '#5A5A5A' }}>NO HAY PROVEEDORES</p>
            </div>
          ) : (
            <div className="space-y-2">
              {filtered.map(c => (
                ...
                >
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-black text-white" style={{ background: 'linear-gradient(135deg, #3B82F6, #1D4ED8)' }}>
                    {(c.razon_social || 'P')[0].toUpperCase()}
                  </div>
          ...
          <div className="p-5">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-sm font-black text-white uppercase tracking-wider">
                {selected ? 'DETALLE DE PROVEEDOR' : 'NUEVO PROVEEDOR'}
              </h2>
            ...
            <div className="w-full h-24 rounded-xl flex items-center justify-center mb-6 glass-card" style={{ border: '1px dashed rgba(255,255,255,0.1)' }}>
              <Truck size={32} style={{ color: '#2A2A2E' }} />
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[9px] font-bold uppercase tracking-wider" style={{ color: '#5A5A5A' }}>RUC</label>
                  <input className="glass-input w-full px-3 py-2 rounded-lg text-[13px] font-mono" value={form.ruc_dni} onChange={e => setForm({...form, ruc_dni: e.target.value.toUpperCase()})} placeholder="20123456789" maxLength={11} />
                </div>
                <div className="space-y-1">
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={handleSave} className="btn-relief-accent flex-1 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider flex items-center justify-center gap-2">
                  <Save size={14} /> {selected ? 'GUARDAR CAMBIOS' : 'CREAR PROVEEDOR'}
                </button>
