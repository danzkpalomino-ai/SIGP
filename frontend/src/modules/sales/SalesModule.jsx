...
import { CheckCircle, ShoppingCart, Trash2, X, CreditCard, Banknote, Smartphone, ArrowRightLeft, MoreHorizontal, FileText, Printer, Receipt, ChevronRight } from 'lucide-react'
    ...
    <div className="h-full flex flex-col" style={{ background: '#0A0A0B' }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden p-4" style={{ borderRight: '1px solid rgba(255,255,255,0.06)' }}>
          <h2 className="text-[11px] font-black uppercase tracking-widest mb-3" style={{ color: '#9E9E9E' }}>AGREGAR PRODUCTO</h2>
          ...
          {searchTab === 'qr' && (
            <div className="mb-3 rounded-xl p-3 flex items-center gap-3" style={{ background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.15)' }}>
              ...
              <div>
                <p className="text-[10px] font-bold text-white">Escanear codigo QR</p>
                <p className="text-[8px]" style={{ color: '#9E9E9E' }}>Usa la camara o ingresa el codigo manualmente</p>
              </div>
            </div>
          )}
          {searchTab === 'manual' && (
            <div className="mb-3 rounded-xl p-3 flex items-center gap-3" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.15)' }}>
              ...
              <div>
                <p className="text-[10px] font-bold text-white">Busqueda manual</p>
                <p className="text-[8px]" style={{ color: '#9E9E9E' }}>Busca por nombre, codigo de producto o marca</p>
              </div>
            </div>
          )}
          ...
          {search && filteredProducts.length > 0 ? (
                  ...
                  <div className="w-full h-28 rounded-xl flex items-center justify-center mb-3 overflow-hidden" style={{ background: 'rgba(10,10,11,0.8)' }}>
                    {product.imagen ? (
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[8px]" style={{ color: '#2D2D34' }}>SIN IMAGEN</p>
                      </div>
                    )}
                  </div>
                  <p className="text-[10px] font-bold text-white line-clamp-2 leading-tight mb-1">{product.descripcion}</p>
                  <p className="text-[9px]" style={{ color: '#9E9E9E' }}>{product.marca || ''}</p>
            ...
            <div className="rounded-2xl p-5 mb-4 animate-slide-up" style={{ background: 'rgba(26,26,29,0.6)', border: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-3">
                      <span className="text-[10px]" style={{ color: '#9E9E9E' }}>Codigo:</span>
                      <span className="text-[11px] font-mono font-bold" style={{ color: '#FF6B00' }}>{selectedProduct.codigo_pos || '—'}</span>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ background: 'rgba(16,185,129,0.15)', color: '#10B981' }}>Stock disponible</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[10px]" style={{ color: '#9E9E9E' }}>Marca:</span>
                      <span className="text-[11px] font-bold text-white">{selectedProduct.marca || '—'}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[10px]" style={{ color: '#9E9E9E' }}>Modelo:</span>
                      <span className="text-[11px] font-bold text-white">{selectedProduct.modelo || selectedProduct.descripcion?.substring(0, 20) || '—'}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[10px]" style={{ color: '#9E9E9E' }}>Almacen:</span>
                      <span className="text-[11px] font-bold text-white">Principal</span>
                    </div>
                  </div>
                  <div className="mt-2">
                    <span className="text-[10px] font-black" style={{ color: '#FF6B00' }}>{selectedProduct.stock_actual ?? 0}</span>
                    <span className="text-[9px] ml-1" style={{ color: '#9E9E9E' }}>unidades</span>
                  </div>
                  <button onClick={() => setShowProductDetail(true)} className="text-[9px] font-bold mt-1 flex items-center gap-1 hover:underline" style={{ color: '#FF6B00' }}>
              ...
              <div className="flex items-center gap-4 mt-4 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                <div>
                  <p className="text-[8px] font-bold uppercase tracking-wider mb-0.5" style={{ color: '#9E9E9E' }}>PRECIO UNITARIO</p>
                  <p className="text-xl font-black" style={{ color: '#FF6B00' }}>S/ {Number(selectedProduct.precio_unitario).toFixed(2)}</p>
                </div>
                <div className="flex items-center gap-3">
                  <p className="text-[8px] font-bold uppercase tracking-wider" style={{ color: '#9E9E9E' }}>CANTIDAD</p>
                ...
                <button onClick={() => { addToCart(selectedProduct, selectedQty); setSelectedQty(1) }}
                  className="flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 text-white transition-all"
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#2D2D34' }}>Busca o escanea un producto</p>
              </div>
            </div>
          )}
          ...
          {!search && (
            <div className="mt-auto">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: '#9E9E9E' }}>PRODUCTOS RECIENTES / POPULARES</p>
                <button onClick={loadAllProducts} className="text-[9px] font-bold flex items-center gap-1" style={{ color: '#FF6B00' }}>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'thin' }}>
                {topProducts.map(product => (
                  <button key={product._id}
                    ...
                    <p className="text-[10px] font-bold text-white line-clamp-2 leading-tight mb-1">{product.descripcion}</p>
                    <p className="text-[11px] font-black" style={{ color: '#FF6B00' }}>S/ {Number(product.precio_unitario).toFixed(2)}</p>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="w-[420px] flex flex-col overflow-hidden" style={{ background: 'rgba(26,26,29,0.3)' }}>
          ...
          <div className="p-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] font-black uppercase tracking-widest text-white">CLIENTE</span>
            </div>
            <div className="flex gap-2 mb-2">
              <div className="flex-1">
                <label className="text-[8px] font-bold uppercase tracking-wider mb-1 block" style={{ color: '#9E9E9E' }}>Nombre</label>
                <div className="relative">
                  <input className="w-full px-3 py-2 rounded-lg text-[12px]"
                    ...
                    <div className="absolute top-full left-0 right-0 z-20 mt-1 rounded-lg overflow-hidden" style={{ background: '#1A1A1E', border: '1px solid rgba(255,255,255,0.08)' }}>
                      {clienteSuggestions.map(c => (
                        <button key={c._id} onClick={() => selectClient(c)}
                          ...
                          <span className="ml-2 text-[9px]" style={{ color: '#9E9E9E' }}>{c.ruc_dni}</span>
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="w-32">
                <label className="text-[8px] font-bold uppercase tracking-wider mb-1 block" style={{ color: '#9E9E9E' }}>DNI</label>
                <input className="w-full px-3 py-2 rounded-lg text-[12px] font-mono"
                  style={{ background: 'rgba(26,26,29,0.6)', border: '1px solid rgba(255,255,255,0.08)', color: '#E0E0E0', outline: 'none' }}
                  placeholder="DNI"
                  value={clienteDni}
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg" style={{ background: 'rgba(255,107,0,0.06)' }}>
              ...
              <span className="text-[9px] font-bold" style={{ color: '#FF6B00' }}>Cliente frecuente</span>
              <span className="ml-auto text-[9px] font-bold" style={{ color: '#FF6B00' }}>125 pts. <ChevronRight size={8} className="inline" /></span>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col overflow-hidden p-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-[11px] font-black uppercase tracking-widest text-white">PRODUCTOS EN VENTA ({cart.length})</p>
              <button onClick={clearCart} className="flex items-center gap-1 text-[9px] font-bold" style={{ color: '#9E9E9E' }}>
            ...
            <div className="flex-1 overflow-y-auto space-y-2" style={{ scrollbarWidth: 'thin' }}>
              {cart.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-[10px] font-bold" style={{ color: '#2D2D34' }}>Carrito vacio</p>
                </div>
              ) : (
                cart.map(item => (
                  <div key={item._id} className="flex items-center gap-3 p-3 rounded-xl"
                    ...
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-bold text-white truncate">{item.descripcion}</p>
                      <p className="text-[9px] font-mono" style={{ color: '#9E9E9E' }}>Codigo: {item.codigo_pos || '—'}</p>
                    </div>
                    {/* Qty Controls */}
                    ...
                    {/* Price */}
                    <p className="text-[11px] font-bold text-white w-20 text-right">S/ {Number(item.precio_unitario).toFixed(2)}</p>
                    <p className="text-[11px] font-black w-20 text-right" style={{ color: '#FF6B00' }}>S/ {item.total_item.toFixed(2)}</p>
                    {/* Remove */}
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => handleCheckout('01')} disabled={cart.length === 0 || loading}
                className="flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all disabled:opacity-30"
              ...
              <button className="px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-1.5 transition-all"
            ...
            <div className="grid grid-cols-6 gap-2 mt-3">
              {[
                { icon: Banknote, label: 'EFECTIVO', color: '#10B981' },
                { icon: CreditCard, label: 'TARJETA', color: '#3B82F6' },
                { icon: Smartphone, label: 'YAPE', color: '#A855F7' },
                { icon: Smartphone, label: 'PLIN', color: '#EC4899' },
                { icon: ArrowRightLeft, label: 'TRANSFERENCIA', color: '#F59E0B' },
                ...
                return (
                  <button key={i} onClick={() => setSelectedPayment(isActive ? null : method.label)}
                    ...
                    <span className="text-[7px] font-bold uppercase tracking-wider" style={{ color: isActive ? method.color : '#9E9E9E' }}>{method.label}</span>
                  </button>
                )
              })}
            </div>
      ...
      <div className="flex items-center justify-between px-5 py-2 shrink-0" style={{ background: 'rgba(26,26,29,0.5)', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-bold" style={{ color: '#9E9E9E' }}>Sistema sincronizado con SICCE ERP</span>
        </div>
        <span className="text-[9px]" style={{ color: '#5A5A5A' }}>Ultima sincronizacion: {new Date().toLocaleString('es-PE')}</span>
        <span className="text-[9px]" style={{ color: '#5A5A5A' }}>Version: 2.0.0</span>
      </div>

      {/* ═══ SUCCESS TOAST ═══ */}
      {successMsg && (
        <div className="fixed top-4 right-4 z-50 px-4 py-2 rounded-xl text-[11px] font-bold animate-slide-up flex items-center gap-2"
          ...
          <div className="rounded-2xl p-6 max-w-md w-full mx-4 animate-slide-up"
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowConfirm(false)}
                className="flex-1 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider"
              ...
              <button onClick={handleConfirmSale} disabled={loading}
                className="flex-1 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-wider text-white disabled:opacity-50 flex items-center justify-center gap-1.5"
